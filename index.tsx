...
async function sendPromptToAI(parts: Part[], userMessageId: string) {
    ...
    try {
        // Attempt with current session (initialized with PRIMARY_MODEL)
        await performGeneration(currentChatSession);
    } catch (error: any) {
        console.warn("Generation failed with primary model. Error:", error);
        
        // Logic to detect if we should fallback (quota exceeded, service unavailable, etc.)
        ...
        if (isQuotaError || isServerUnavailable) {
            ...
            try {
                // Reset text and recreate session with FALLBACK_MODEL
                fullResponseText = ''; 
                ...
                if (session) {
                    ...
                    throw error; // Re-throw if no session found to fallback
                }
            } catch (fallbackError) {
